...
const Navbar = () => {
  ...
  return (
    <nav className="bg-primary border-b-2 border-accent sticky top-0 z-50 shadow-lg" role="navigation" aria-label="Main navigation">
            ...
            {navLinks
              ...
              .map((link) => (
                <Link
                  key={link.to}
                  to={link.to}
                  className="text-text hover:text-accent hover:bg-primary-dark flex items-center space-x-2 px-4 py-2 rounded-lg text-base font-bold transition-all duration-200 tracking-wide min-h-[44px]"
                >
          ...
          <div className="flex items-center space-x-2 sm:space-x-4">
            ...
            {isAuthenticated ? (
              <div className="relative">
                ...
                {isProfileMenuOpen && (
                  <div className="absolute right-0 mt-2 w-52 bg-white border-accent border-2 rounded-lg p-2 space-y-1 shadow-xl" role="menu">
                    <Link
                      to="/profile"
                      className="block text-text hover:text-accent px-4 py-3 rounded-lg font-bold text-sm transition-all duration-200 hover:bg-primary-dark min-h-[44px] flex items-center"
                    ...
                    <button
                      ...
                      className="block w-full text-left text-text hover:text-accent px-4 py-3 rounded-lg font-bold text-sm transition-all duration-200 hover:bg-primary-dark min-h-[44px] flex items-center"
                      role="menuitem"
                    >
              ...
              <div className="flex items-center space-x-2">
                <Link
                  to="/login"
                  className="text-text border-text border-2 px-3 sm:px-4 py-2 rounded-lg font-bold text-sm sm:text-base hover:bg-primary-dark transition-all duration-200 min-h-[44px] flex items-center"
                >
                  {t('nav.login')}
                </Link>
                <Link
            ...
            {navLinks
              ...
              .map((link) => (
                <Link
                  key={link.to}
                  to={link.to}
                  className="block text-text hover:text-accent px-4 py-3 rounded-lg font-bold text-base border-l-4 border-accent hover:bg-primary-dark transition-all duration-200 min-h-[44px]"
