...
const AddPlaceModal = ({ isOpen, onClose, onSuccess, editPlace = null, preselectedCountryCode = null }) => {
  ...
  return (
    <dialog open className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4">
      ...
      {/* Modal Container - Más ancho para el selector de países */}
      <div className="relative w-full max-w-3xl bg-white rounded-lg shadow-2xl flex flex-col max-h-[95vh]">
        {/* Header - Compacto */}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden" aria-labelledby="add-place-modal-title">
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-5 py-4 space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {statusOptions.map(opt => (
                    <button
                      ...
                      className={`px-3 py-2.5 text-sm font-medium rounded-lg border-2 transition-all duration-200 cursor-pointer ${
                        formData.status === opt.value
                          ? 'bg-[#213638] text-white border-[#213638] shadow-lg scale-105'
                          : 'bg-white text-gray-700 border-gray-300 hover:bg-[#E5A13E]/20 hover:text-[#213638] hover:border-[#E5A13E] hover:scale-105 hover:shadow-md'
                      }`}
                    >
                      {opt.label}
                    </button>
            ...
            <div>
              ...
              <div className="flex items-center gap-1" role="group" aria-label="Valoración del lugar">
                {[1, 2, 3, 4, 5].map(star => (
                  <button
                    key={star}
                    type="button"
                    ...
                    className="p-1 rounded-full cursor-pointer transition-all duration-200 hover:scale-125 hover:bg-[#E5A13E]/10 focus:outline-none focus:ring-2 focus:ring-[#E5A13E] focus:ring-offset-1"
                  >
                    <svg
                      className={`w-6 h-6 transition-colors duration-200 ${star <= formData.rating ? 'text-[#E5A13E]' : 'text-gray-300 hover:text-[#E5A13E]/60'}`}
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
          ...
          <div className="flex-shrink-0 px-5 py-4 bg-gray-50 border-t border-gray-200 rounded-b-lg">
            <div className="flex flex-col gap-3 sm:flex-row sm:gap-3 w-full">
              <button
                type="button"
                onClick={onClose}
                className="w-full sm:w-1/2 px-5 py-3 text-sm font-semibold text-gray-700 bg-white border-2 border-gray-300 rounded-lg cursor-pointer hover:bg-gray-100 hover:border-gray-400 hover:shadow-md transition-all duration-200"
              >
                Cancelar
              </button>
              <button
                ...
                className={`w-full sm:w-1/2 px-5 py-3 text-sm font-semibold rounded-lg transition-all duration-200 ${
                  loading || !selectedCountry
                    ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                    : 'bg-[#213638] text-white cursor-pointer hover:bg-[#2d4a4d] hover:shadow-lg hover:scale-[1.02]'
                }`}
              >
                {loading ? (
