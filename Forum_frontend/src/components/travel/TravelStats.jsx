...
const TravelStats = ({ stats }) => {
  ...
  return (
    <div className="bg-gradient-to-br from-success-dark to-dark rounded-2xl p-6 text-white shadow-xl">
      {/* Header con nivel */}
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-2xl font-bold">{stats.username}</h3>
          <p className="text-white/90 font-semibold">{stats.travelerLevel}</p>
        </div>
        {stats.globalRanking && (
          <div className="text-right">
            <span className="text-4xl font-bold">#{stats.globalRanking}</span>
            <p className="text-white/90 text-sm font-semibold">Ranking Global</p>
          </div>
        )}
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="h-4 bg-white/20 rounded-full overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-warning-dark to-warning rounded-full transition-all duration-1000"
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {stats.badges.slice(0, 6).map((badge) => (
              <span 
                key={badge}
                className="px-3 py-1 bg-warning-dark/30 border border-warning-dark/50 rounded-full text-sm text-white font-semibold"
                title={badge}
              >
                {badge}
              </span>
      ...
      {stats.favoritePlace && (
        <div className="mt-4 pt-4 border-t border-white/20">
          <span className="text-white/80 text-sm font-semibold">Lugar favorito: </span>
          <span className="font-bold">❤️ {stats.favoritePlace}</span>
        </div>
      )}
    </div>
  )
...
const StatCard = ({ icon, value, label, subLabel }) => (
  <div className="bg-white/10 rounded-xl p-4 text-center backdrop-blur-sm">
    ...
    <div className="text-2xl font-bold mt-1">
      {value}
      {subLabel && <span className="text-sm font-normal text-white/70">{subLabel}</span>}
    </div>
